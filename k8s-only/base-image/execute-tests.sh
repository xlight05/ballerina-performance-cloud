...
cat /etc/hosts
echo "Starting sed"
sed '/bal\.perf\.test/d' /etc/hosts |  tee /etc/hosts
echo "$cluster_ip bal.perf.test" |  tee -a /etc/hosts
echo "hosts file"
cat /etc/hosts
echo "Host name"
echo $HOSTNAME
...
echo "--------Generating CSV--------"
bash /home/root/apache-jmeter-5.4/bin/JMeterPluginsCMD.sh --generate-csv summary.csv --input-jtl original-measurement.jtl --plugin-type AggregateReport
echo "--------CSV generated--------"
